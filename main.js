...
function toggleContrast() {
  ...
  document.getElementById('btn-contrast').textContent =
    document.body.classList.contains('high-contrast') ? '⚪ Desativar' : '👁 Contraste';
}
...
function undo(team) {
  if (!history[team].length) return;
  const last = history[team].pop();
...
function updateHistory() {
  ...
  const allMoves = [
    ...history.A.map((p, i) => ({ team: 'BULLS',  pts: p, idx: i * 2 })),
    ...history.B.map((p, i) => ({ team: 'LAKERS', pts: p, idx: i * 2 + 1 }))
...
function nextQuarter() {
  if (quarter < quarters.length - 1) quarter++;
  document.getElementById('quarterDisplay').textContent = quarters[quarter - 1] || 'PRORROGAÇÃO';
}
...
function resetGame() {
  ...
  document.getElementById('quarterDisplay').textContent = '1º QUARTO';
  updateHistory();
}
...
const questions = [
  ...
  },
  {
    ...
    options: ['Larry Bird', 'James Naismith', 'Bill Russell', 'John Wooden'],
    answer: 1,
  ...
  },
  {
    ...
    options: ['81 pontos', '92 pontos', '100 pontos', '73 pontos'],
    answer: 2,
  ...
  },
  {
    ...
    options: ['The Flash', 'Black Mamba', 'The King', 'Air'],
    answer: 1,
...
function renderQuestion() {
  ...
  document.getElementById('quizNum').textContent      = `Pergunta ${currentQ + 1} de ${questions.length}`;
  document.getElementById('quizQuestion').textContent = q.q;
  document.getElementById('quizProgress').style.width = `${(currentQ / questions.length) * 100}%`;
  document.getElementById('quizScoreText').textContent = `Pontuação: ${quizScore}`;
...
function selectAnswer(idx) {
  ...
  if (idx === q.answer) {
    ...
    fb.innerHTML = `❌ <strong>Errou!</strong> ${q.feedback}`;
  }
  document.getElementById('quizScoreText').textContent = `Pontuação: ${quizScore}`;
...
function showResult() {
  ...
  document.getElementById('resultScore').textContent = `${quizScore} / ${questions.length}`;
}
...
function updateComparison() {
  const a = playerData[document.getElementById('playerA').value];
  const b = playerData[document.getElementById('playerB').value];
  const container = document.getElementById('comparadorStats');
